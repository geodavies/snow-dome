...
public class LessonControllerTest extends AbstractControllerTest {
  ...
  public void getLessonsLoggedInUser() throws Exception {
    this.mockMvc.perform(get("/lessons/available")
        .session(super.getNewMockHttpSession(true)))
        .andExpect(status().isOk())
        .andExpect(view().name("/availableLessons"));
  }
  ...
  public void getLessonsNonLoggedInUser() throws Exception {
    this.mockMvc.perform(get("/lessons/available")
        ...
        .andExpect(
            content().string(containsString("You must be logged in to access that resource")));
  }
  ...
  public void selectLessonMaximumExceeded() throws Exception {
    ...
    Set<Lesson> selectedLessons = new HashSet<>(
        Arrays.asList(testLesson1, testLesson2, testLesson3));
    ...
    this.mockMvc.perform(post("/lessons/select")
        ...
        .andExpect(content().string(containsString("Maximum number of selected lessons exceeded")));
  }
  ...
  public void selectLessonNotFound() throws Exception {
    this.mockMvc.perform(post("/lessons/select")
        ...
        .andExpect(content().string(containsString("Lesson not found")));
  }
