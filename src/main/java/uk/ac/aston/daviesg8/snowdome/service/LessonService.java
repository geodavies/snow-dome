...
public class LessonService {
    ...
    /**
     * Finds the given lesson from the database and adds it to the selected lessons in the session. If the lesson
     * doesn't exist then an exception is thrown. If more than 3 lessons have been selected then an exception will also
     * be thrown.
     *
     ...
     */
    public void addLessonToSession(HttpSession httpSession, String lessonId) throws LessonNotFoundException, MaximumSelectedLessonsExceededException {
    ...
    /**
     * Looks for all selected lessons currently in the session which match the provided lessonId and removes them from
     * the session.
     *
    ...
    public void removeLessonFromSession(HttpSession httpSession, String lessonId) {
        ...
        Set<Lesson> currentlySelectedLessons = (Set<Lesson>) httpSession.getAttribute("selectedLessons");
    ...
    /**
     * Adds the lesson to the session. If there are currently none selected then it will create a new set. This also
     * checks the maximum limit on lessons per user.
     *
     * @param httpSession the session to add lesson to
     * @param lesson      the lesson to add to the session
     * @throws MaximumSelectedLessonsExceededException more than 3 lessons selected
     */
    private void addLessonToSession(HttpSession httpSession, Lesson lesson) throws MaximumSelectedLessonsExceededException {
