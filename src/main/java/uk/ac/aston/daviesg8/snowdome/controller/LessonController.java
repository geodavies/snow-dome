...
public class LessonController extends AbstractAuthenticatedController {
    ...
    public String getAllLessons(HttpSession httpSession, ModelMap modelMap) throws ClientNotLoggedInException {
    ...
    /**
     * Handles a post request to select a single lesson. This will first check that the lesson is in the database and
     * then add it to the current session. However if the client has already selected 3 lessons then the new one will
     * not be added.
     *
    ...
    public String getSelectedLessons(HttpSession httpSession, ModelMap modelMap) throws ClientNotLoggedInException {
        super.checkLoggedIn(httpSession);
